...
public class Servlet_Update extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String enumberupdate = request.getParameter("Number");
            employee.setId(id1);
            employee.setEname(ename);
            employee.setEnumber(enumberupdate);
            Session session = NewHibernateUtil.getSessionFactory().openSession(); // Ouvre le tuyau vers mysql
            Transaction transaction = session.beginTransaction(); // Début de Transaction
            session.update(employee); // Mise à jour  dans la BDD 
